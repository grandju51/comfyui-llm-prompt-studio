...
TEMPLATES = {
    ...
    "MiniMax H3 - ref (full-reference)": (
        ...
        "below is a formatting example, not an asset you received. <Video N> exists "
        "only if the user describes such a source. <Audio 1> exists only when an "
        "[AUDIO INPUT] line below declares it, or when the user describes a source "
        "track; without either, never cite it.\n"
